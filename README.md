...
Out of the box, `CNCHStateMachine` provides a KVO-observable `state` property.  This works fine during early prototyping, but as your state machine grows in size and complexity, getting meaningful feedback via KVO is bound to become complex and difficult to maintain.  Rather, we recommend implementing a `CNCHStateMachine` subclass, adding whatever delegate properties and methods you see fit.  Additionally, we recommend creating an analagous sub-protocol of `CNCHStateful` and updating the relevant type specifiers accordingly.
